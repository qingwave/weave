...
LDFLAGS = -X weave/pkg/version.gitVersion=$(GIT_VERSION) \
	-X weave/pkg/version.gitCommit=$(GIT_COMMIT) \
	-X weave/pkg/version.gitBranch=$(GIT_BRANCH) \
	-X weave/pkg/version.gitTreeState=$(GIT_STATE) \
	-X weave/pkg/version.buildDate=$(BUILD_DATE)

PKGS = $(shell go list ./...)
GOFILES = $(shell find . -name "*.go" -type f -not -path "./vendor/*")

base: clean test swagger fmt
